...
path = "/ceph/awiedl/FCCee/HiggsCP/stage2_v2_cut/QQ/HH/"

output_file = "/ceph/sgiappic/FCCAnalyses/examples/FCCee/higgs/tautau/xsec/BDT/outputs_new/output_QQHH.txt"
...
if train_tree == True:
    ...
    # Save the figure
    fig.savefig("/web/sgiappic/public_html/Higgs_xsec/BDT_v2/ROC_new/QQHH_ROC.pdf")

    #Write model to joblib file
    joblib.dump(bdt, f"{out}/xgb_bdt_QQHH.joblib")
    ...
    ROOT.TMVA.Experimental.SaveXGBoost(bdt, "Htautau", f"{out}/xgb_bdt_QQHH.root", num_inputs=len(vars_list))

    bdt.save_model(f"{out}/xgb_bdt_QQHH_model.json")
